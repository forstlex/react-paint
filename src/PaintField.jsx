...
/**
 * Paint Tool based on FabricJS for React Applications
 */
class PaintField extends PureComponent {
  ...
  static propTypes = {
    ...
    // the background color of the paint
    ...
    // Paint data for controlling paint from
  ...
  /**
   * Sets the background color for this paint
  ...
  createBackgroundGridDataUrl =() => {
    var canvas = this._fc;
    var grid = 10;
    this.setState({emptyData: canvas.toDataURL()});
    // create grid
    var canvaWidth = canvas.getWidth();
    var canvaHeight = canvas.getHeight();

    for (var i = 0; i < (canvaWidth / grid); i++) 
      canvas.add(new fabric.Line([ i * grid, 0, i * grid, canvaWidth], { stroke: '#ccc', selectable: false }));

    for (var j = 0; j < (canvaHeight / grid); j++)
      canvas.add(new fabric.Line([ 0, j * grid, canvaWidth, j * grid], { stroke: '#ccc', selectable: false }));
  ...
  setBackgroundGrid = (gridon) =>{
    var canvas = this._fc;
    canvas.selection = false;

    if (gridon)
      this.setBackgroundFromDataUrl(this.state.imageData);
    else
      this.setBackgroundFromDataUrl(this.state.emptyData);
  };
  ...
  setBackgroundFromDataUrl = (dataUrl, options = {}) => {
    ...
    const { gridon, ...fabricOptions } = options
    img.onload = () => {
  ...
  componentDidMount = () => {
    ...
    this.createBackgroundGridDataUrl();
  ...
  render = () => {
    ...
    let canvasDivStyle = Object.assign({}, style ? style : {},
      width ? { width: width } : {},
      height ? { height: height } : { height: 890 });
    return (
      <div
        className={className}
